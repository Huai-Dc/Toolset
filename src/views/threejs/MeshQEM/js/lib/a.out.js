...
function alignMemory(size, factor) {
    ...
    let ret = size = Math.ceil(size / factor) * factor;
    return ret;
}
    ...
    Module['reallocBuffer'] = (function (size) {
        ...
        try {
            if (ArrayBuffer.transfer) {
                ...
                let temp = new Int8Array(ret);
                temp.set(oldHEAP8);
            }
        } catch (e) {
            return false;
        }
        let success = _emscripten_replace_memory(ret);
...
function integrateWasmJS() {
    let wasmTextFile = 'a.out.wast';
    let wasmBinaryFile = 'a.out.wasm';
    let asmjsCodeFile = 'a.out.temp.asm.js';
        ...
        get_char: (function (tty) {
            if (!tty.input.length) {
                ...
                if (ENVIRONMENT_IS_NODE) {
                    ...
                    let bytesRead = 0;
                    let isPosixPlatform = process.platform != 'win32';
...
let MEMFS = {
    ...
    stream_ops: {
        ...
        msync: (function (stream, buffer, offset, length, mmapFlags) {
            ...
            let bytesWritten = MEMFS.stream_ops.write(stream, buffer, 0, length, offset, false);
            return 0;
        })
    }
};
let IDBFS = {
    ...
    getRemoteSet: (function (mount, callback) {
        ...
        IDBFS.getDB(mount.mountpoint, (function (err, db) {
            if (err) return callback(err);
            try {
                let transaction = db.transaction([IDBFS.DB_STORE_NAME], 'readonly');
                transaction.onerror = (function (e) {
                    callback(this.error);
                    e.preventDefault();
                });
                let store = transaction.objectStore(IDBFS.DB_STORE_NAME);
    ...
    reconcile: (function (src, dst, callback) {
        ...
        let transaction = db.transaction([IDBFS.DB_STORE_NAME], 'readwrite');
        let store = transaction.objectStore(IDBFS.DB_STORE_NAME);
...
let NODEFS = {
    ...
    flagsForNode: (function (flags) {
        ...
        flags &= ~524288;
        let newFlags = 0;
        for (let k in NODEFS.flagsForNodeMap) {
            if (flags & k) {
    ...
    node_ops: {
        ...
        lookup: (function (parent, name) {
            let path = PATH.join2(NODEFS.realPath(parent), name);
            let mode = NODEFS.getMode(path);
            return NODEFS.createNode(parent, name, mode);
        }),
        mknod: (function (parent, name, mode, dev) {
            let node = NODEFS.createNode(parent, name, mode, dev);
            let path = NODEFS.realPath(node);
            try {
                if (FS.isDir(node.mode)) {
        ...
        rename: (function (oldNode, newDir, newName) {
            let oldPath = NODEFS.realPath(oldNode);
            let newPath = PATH.join2(NODEFS.realPath(newDir), newName);
            try {
        ...
        symlink: (function (parent, newName, oldPath) {
            let newPath = PATH.join2(NODEFS.realPath(parent), newName);
            try {
    ...
    stream_ops: {
        open: (function (stream) {
            let path = NODEFS.realPath(stream.node);
            try {
                if (FS.isFile(stream.node.mode)) {
        ...
        llseek: (function (stream, offset, whence) {
            ...
            if (whence === 1) {
                ...
                if (FS.isFile(stream.node.mode)) {
                    try {
                        let stat = fs.fstatSync(stream.nfd);
                        position += stat.size;
                    } catch (e) {
                        throw new FS.ErrnoError(ERRNO_CODES[e.code]);
                    }
...
let WORKERFS = {
    ...
    mount: (function (mount) {
        ...
        let root = WORKERFS.createNode(null, '/', WORKERFS.DIR_MODE, 0);
        let createdParents = {};
        ...
        (mount.opts['packages'] || []).forEach((function (pack) {
            pack['metadata'].files.forEach((function (file) {
                let name = file.filename.substr(1);
    ...
    stream_ops: {
        read: (function (stream, buffer, offset, length, position) {
            ...
            let chunk = stream.node.contents.slice(position, position + length);
            let ab = WORKERFS.reader.readAsArrayBuffer(chunk);
...
let FS = {
    ...
    lookupPath: (function (path, opts) {
        ...
        let parts = PATH.normalizeArray(path.split('/')
            .filter((function (p) {
                return !!p;
            })), false);
        let current = FS.root;
        ...
        for (let i = 0; i < parts.length; i++) {
            ...
            if (!islast || opts.follow) {
                ...
                while (FS.isLink(current.mode)) {
                    ...
                    let lookup = FS.lookupPath(current_path, { recurse_count: opts.recurse_count });
                    current = lookup.node;
                    if (count++ > 40) {
                        throw new FS.ErrnoError(ERRNO_CODES.ELOOP);
                    }
    ...
    mayLookup: (function (dir) {
        let err = FS.nodePermissions(dir, 'x');
        if (err) return err;
    ...
    mayDelete: (function (dir, name, isdir) {
        ...
        let err = FS.nodePermissions(dir, 'wx');
        if (err) {
            return err;
        }
        if (isdir) {
    ...
    mknod: (function (path, mode, dev) {
        ...
        let err = FS.mayCreate(parent, name);
        if (err) {
    ...
    symlink: (function (oldpath, newpath) {
        ...
        let err = FS.mayCreate(parent, newname);
        if (err) {
            throw new FS.ErrnoError(err);
        }
    ...
    rmdir: (function (path) {
        ...
        let name = PATH.basename(path);
        let node = FS.lookupNode(parent, name);
        let err = FS.mayDelete(parent, name, true);
        if (err) {
            throw new FS.ErrnoError(err);
        }
    ...
    truncate: (function (path, len) {
        ...
        let err = FS.nodePermissions(node, 'w');
        if (err) {
            throw new FS.ErrnoError(err);
        }
        node.node_ops.setattr(node, {
    ...
    chdir: (function (path) {
        ...
        let err = FS.nodePermissions(lookup.node, 'x');
        if (err) {
            throw new FS.ErrnoError(err);
        }
    ...
    createSpecialDirectories: (function () {
        ...
        FS.mount({
            mount: (function () {
                let node = FS.createNode('/proc/self', 'fd', 16384 | 511, 73);
                node.node_ops = {
                    lookup: (function (parent, name) {
                        let fd = +name;
                        let stream = FS.getStream(fd);
    ...
    createStandardStreams: (function () {
        ...
        let stdout = FS.open('/dev/stdout', 'w');
        assert(stdout.fd === 1, 'invalid handle for stdout (' + stdout.fd + ')');
        let stderr = FS.open('/dev/stderr', 'w');
    ...
    createPath: (function (parent, path, canRead, canWrite) {
        ...
        while (parts.length) {
            let part = parts.pop();
            if (!part) continue;
            let current = PATH.join2(parent, part);
            try {
                FS.mkdir(current);
            } catch (e) {
            }
    ...
    createDataFile: (function (parent, name, data, canRead, canWrite, canOwn) {
        let path = name ? PATH.join2(typeof parent === 'string' ? parent : FS.getPath(parent), name) : parent;
        let mode = FS.getMode(canRead, canWrite);
    ...
    createLazyFile: (function (parent, name, url, canRead, canWrite) {
        ...
        LazyUint8Array.prototype.cacheLength = function LazyUint8Array_cacheLength() {
            ...
            let header;
            let hasByteServing = (header = xhr.getResponseHeader('Accept-Ranges')) && header === 'bytes';
    ...
    createPreloadedFile: (function (parent, name, url, canRead, canWrite, onload, onerror, dontCreateFile, canOwn, preFinish) {
        Browser.init();
        let fullname = name ? PATH.resolve(PATH.join2(parent, name)) : parent;
    ...
    loadFilesFromDB: (function (paths, onload, onerror) {
        ...
        openRequest.onsuccess = function openRequest_onsuccess() {
            let db = openRequest.result;
            try {
                let transaction = db.transaction([FS.DB_STORE_NAME], 'readonly');
            } catch (e) {
                onerror(e);
                return;
            }
...
let SYSCALLS = {
    ...
    calculateAt: (function (dirfd, path) {
        if (path[0] !== '/') {
            ...
            if (dirfd === -100) {
                ...
                let dirstream = FS.getStream(dirfd);
    ...
    doMsync: (function (addr, stream, len, flags) {
        let buffer = new Uint8Array(HEAPU8.subarray(addr, addr + len));
    ...
    doReadlink: (function (path, buf, bufsize) {
        ...
        let ret = FS.readlink(path);
        let len = Math.min(bufsize, lengthBytesUTF8(ret));
        let endChar = HEAP8[buf + len];
    ...
    doDup: (function (path, flags, suggestFD) {
        let suggest = FS.getStream(suggestFD);
    ...
    getStr: (function () {
        let ret = Pointer_stringify(SYSCALLS.get());
        return ret;
    }),
    getStreamFromFD: (function () {
        let stream = FS.getStream(SYSCALLS.get());
        if (!stream) throw new FS.ErrnoError(ERRNO_CODES.EBADF);
        return stream;
    }),
    getSocketFromFD: (function () {
        let socket = SOCKFS.getSocket(SYSCALLS.get());
        if (!socket) throw new FS.ErrnoError(ERRNO_CODES.EBADF);
        return socket;
    }),
    getSocketAddress: (function (allowNull) {
        let addrp = SYSCALLS.get(),
            addrlen = SYSCALLS.get();
        if (allowNull && addrp === 0) return null;
        let info = __read_sockaddr(addrp, addrlen);
...
function ___syscall5(which, varargs) {
    SYSCALLS.varargs = varargs;
    try {
        let pathname = SYSCALLS.getStr(),
            flags = SYSCALLS.get(),
            mode = SYSCALLS.get();
        let stream = FS.open(pathname, flags, mode);
        return stream.fd;
    } catch (e) {
        if (typeof FS === 'undefined' || !(e instanceof FS.ErrnoError)) abort(e);
        return -e.errno;
...
Module['asm'] = asm;
let __GLOBAL__sub_I_Main_cpp = Module['__GLOBAL__sub_I_Main_cpp'] = (function () {
    return Module['asm']['__GLOBAL__sub_I_Main_cpp'].apply(null, arguments);
});
let ___cxa_demangle = Module['___cxa_demangle'] = (function () {
    return Module['asm']['___cxa_demangle'].apply(null, arguments);
});
let ___errno_location = Module['___errno_location'] = (function () {
    return Module['asm']['___errno_location'].apply(null, arguments);
});
let _emscripten_replace_memory = Module['_emscripten_replace_memory'] = (function () {
...
let _simplify = Module['_simplify'] = (function () {
